...
3) A `camonitor` thread which monitors block names and run control and adds updates to queue
...
3) The worker thread (1) picks this up and then: 
   ...
   - If successful, sets dataclass hardware running state to `1`
...
As well as this, the thread calling PV posts (5) updates PVs continuously (in this case the hardware running one). 
